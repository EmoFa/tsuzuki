...
anitui reads a TOML file. Every key is optional: anything you leave out uses the
default shown below, so a config file only needs the settings you want to change.

```sh
anitui config init    # write a commented config with every default
anitui config edit    # open it in $VISUAL / $EDITOR
anitui config check   # validate it
anitui config path    # show where the config, database and log live
```

Changes take effect the next time anitui starts. An unknown key or invalid value
...
| | Linux | macOS | Windows |
|---|---|---|---|
| Config (`config.toml`, AniList login) | `~/.config/anitui` | `~/Library/Application Support/anitui` | `%AppData%\anitui` |
| Data (`anitui.db`: history, list, mappings) | `~/.local/share/anitui` | `~/Library/Application Support/anitui` | `%LocalAppData%\anitui` |
| Cache (`anitui.log`, downloaded Chromium) | `~/.cache/anitui` | `~/Library/Caches/anitui` | `%LocalAppData%\anitui` |

On Linux, `XDG_CONFIG_HOME`, `XDG_DATA_HOME` and `XDG_CACHE_HOME` are respected.
Each directory can also be moved with `ANITUI_CONFIG_DIR`, `ANITUI_DATA_DIR` or
`ANITUI_CACHE_DIR`, and `--config <file>` reads a different config file.
...
| Key | Default | Description |
|---|---|---|
| `player.mpv_path` | `""` | Path to the mpv binary. Empty searches `PATH` (and the usual install locations on macOS and Windows). |
| `player.extra_args` | `[]` | Extra mpv arguments, e.g. `["--fullscreen", "--volume=70"]`. They come after anitui's own arguments, so they win: avoid `--start` (it overrides resuming), `--input-ipc-server` and `--idle`, which anitui relies on. mpv's own `mpv.conf` is also read unless you pass `--no-config`. |

## `[browser]`

anitui uses Chrome or Chromium for two things: running the embedded players some
...
| `browser.headless` | `true` | Try bot checks invisibly first. When that doesn't pass (or with `false`), a normal browser window opens for you to complete the check; anitui never clicks it for you. |
...
| Key | Default | Description |
|---|---|---|
| `tracking.backend` | `"anilist"` | `"anilist"`: while you're logged in (`anitui login`), your list mirrors AniList. Watched episodes and status changes are sent straight away, and queued to retry when AniList can't be reached. Before you log in, progress is only kept locally. `"local"`: nothing is ever sent to AniList. |
| `tracking.anilist_client_id` | `0` | AniList API client used by `anitui login`. `0` uses anitui's own client. To use your own, create one at <https://anilist.co/settings/developer> with the redirect URL `http://localhost:47281/callback`. |
...
| `skip.filler_episodes` | `false` | Pass over filler episodes when anitui chooses the episode (`continue` and autoplay). An episode you pick yourself always plays. Mixed canon/filler episodes are never skipped. |
...
| `discord.client_id` | `""` | Discord application ID. Empty uses anitui's application. With your own (from <https://discord.com/developers/applications>), Discord shows your application's name instead of "anitui". |
